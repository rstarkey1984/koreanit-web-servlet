...
## 📘 학습 개요
브라우저에서 웹사이트 호출 시 어떤 과정을 통해 최종적으로 화면에 표시가 되는지 알아보자.

![클라이언트-서버 모델](https://lh3.googleusercontent.com/d/1Ytos16Ll_nOcL2KuZw-c8Yv5QvYfIsdt)

## 1. 브라우저에서 `www.google.com` 을 입력했을 때의 전체 흐름

### 1️⃣ URL 입력

사용자가 브라우저 주소창에 `www.google.com` 을 입력하면, 브라우저는 이것이 **도메인 이름**이라는 것을 인식합니다.

`https://` 가 생략되어 있다면 기본적으로는 HTTP로 요청을 시도하지만, Chrome/Edge 등 최신 브라우저에서는 HTTPS를 먼저 시도하거나, HSTS 정책에 따라 바로 HTTPS로 요청합니다.
