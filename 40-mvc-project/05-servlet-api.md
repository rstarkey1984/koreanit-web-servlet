  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    try {
      ...
      if (path == null || "/".equals(path)) {
        ...
        // 기본은 size 사용, 없으면 limit(이전 코드 호환용)
        int size = parseInt(req.getParameter("size"), 0);
        if (size <= 0) {
          size = parseInt(req.getParameter("limit"), 10);
        }
        ...
        data.put("items", list); // 실제 목록
        data.put("page", page); // 현재 페이지
        data.put("size", size); // 페이지 크기
